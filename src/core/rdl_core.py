...
class RDLParser:
    ...
    def __init__(self, rdl_file_path: str):
        ...
        self.rdl_file_path = self._normalize_path(rdl_file_path)
        
        try:
            self.tree = ET.parse(self.rdl_file_path)
            self.root = self.tree.getroot()
        except ET.ParseError as e:
            raise Exception(f"Invalid XML format in RDL file: {str(e)}")
        except FileNotFoundError:
            raise Exception(f"RDL file not found: {rdl_file_path}")
        except PermissionError:
            raise Exception(f"Permission denied reading RDL file: {rdl_file_path}")
        except Exception as e:
            raise Exception(f"Failed to parse RDL file: {str(e)}")
    ...
    def _normalize_path(self, path: str) -> str:
        ...
        if platform.system() == 'Windows':
            ...
            # Handle UNC paths and long path names
...
class PowerBIConverter:
    ...
    def convert_sql_to_powerquery(self, sql_query: str, dataset_name: str, data_source_info: dict) -> str:
        ...
        if not sql_query or not sql_query.strip():
            raise Exception(f"Empty or invalid SQL query for dataset '{dataset_name}'")
        
        if not dataset_name or not dataset_name.strip():
            raise Exception("Dataset name is required for Power Query conversion")
        ...
        **Format:** Return only the M code, ready to paste into Power BI Advanced Editor.
        ...
        try:
            ...
            raise Exception(f"AI conversion failed for dataset '{dataset_name}': {str(e)}")
...
*/"""
    ...
    def convert_table_to_dax(self, table_element: ReportElement) -> str:
        ...
        # Validate inputs
        if not table_element or not table_element.name:
            raise Exception("Invalid table element provided for DAX conversion")
        
        if table_element.type != 'Table':
            raise Exception(f"Expected table element, got '{table_element.type}' for '{table_element.name}'")
        ...
        **Format:** Return clean DAX code with detailed comments, ready to copy into Power BI.
        ...
        try:
            ...
            raise Exception(f"AI DAX generation failed for table '{table_element.name}': {str(e)}")
...
def create_migration_output_structure() -> dict:
    ...
    # Create main migration directory with Windows compatibility
    if platform.system() == 'Windows':
        # Use Windows-appropriate directory
        migration_dir = Path(os.path.expandvars('%USERPROFILE%\\Documents\\PowerBI_Migration'))
    else:
    ...
    try:
        ...
        if platform.system() == 'Windows' and WINDOWS_AUTH_AVAILABLE:
            # Set appropriate Windows permissions
            _set_windows_directory_permissions(str(migration_dir))
    except PermissionError as e:
        logger.warning(f"Could not create preferred directory {migration_dir}, using fallback: {e}")
        migration_dir = Path('./migration_output')
        migration_dir.mkdir(parents=True, exist_ok=True)
...
def _set_windows_directory_permissions(directory_path: str):
    """Set appropriate Windows permissions for migration output directory"""
    ...
    try:
        ...
        # Create security descriptor with full control for current user
        ...
        logger.info(f"Set Windows permissions for directory: {directory_path}")
    except Exception as e:
...
def save_migration_files(conversion_results: dict, rdl_info: dict, output_dirs: dict):
    """Save all migration files to organized directory structure with Windows compatibility"""
    
    # Save Power Query files with Windows-safe filenames
    for filename, content in conversion_results.get('power_query', {}).items():
        safe_filename = _sanitize_windows_filename(f"{filename}.m")
        file_path = output_dirs['power_query'] / safe_filename
        try:
            ...
            alt_path = Path.cwd() / 'fallback_output' / safe_filename
            alt_path.parent.mkdir(parents=True, exist_ok=True)
            with open(alt_path, 'w', encoding='utf-8', newline='\r\n') as f:
                f.write(content)
            print(f"   ✅ Power Query saved (fallback): {alt_path}")
    
    # Save DAX files with Windows-safe filenames
    for filename, content in conversion_results.get('dax_measures', {}).items():
        safe_filename = _sanitize_windows_filename(f"{filename}.dax")
        file_path = output_dirs['dax_measures'] / safe_filename
        try:
            with open(file_path, 'w', encoding='utf-8', newline='\r\n') as f:
                f.write(content)
            ...
            with open(alt_path, 'w', encoding='utf-8', newline='\r\n') as f:
                f.write(content)
            print(f"   ✅ DAX measures saved (fallback): {alt_path}")
    
    # Save visual configurations with Windows-safe filenames
    for filename, content in conversion_results.get('visual_configs', {}).items():
        safe_filename = _sanitize_windows_filename(f"{filename}.json")
        file_path = output_dirs['visual_configs'] / safe_filename
        try:
            with open(file_path, 'w', encoding='utf-8', newline='\r\n') as f:
            ...
            with open(alt_path, 'w', encoding='utf-8', newline='\r\n') as f:
                json.dump(content, f, indent=2)
            print(f"   ✅ Visual config saved (fallback): {alt_path}")
...
def _sanitize_windows_filename(filename: str) -> str:
    ...
    # Limit filename length (Windows has 255 character limit for filenames)
...
def check_windows_environment():
    """Check Windows environment and log system information"""
    if platform.system() == 'Windows':
        logger.info(f"Running on Windows {platform.release()} {platform.version()}")
        logger.info(f"Python version: {sys.version}")
        logger.info(f"Windows authentication available: {WINDOWS_AUTH_AVAILABLE}")
        
        # Check if running under IIS
        if 'WSGI_HANDLER' in os.environ:
            logger.info("Running under IIS/WSGI")
        elif 'SERVER_SOFTWARE' in os.environ:
            logger.info(f"Web server: {os.environ['SERVER_SOFTWARE']}")
        
        # Log important Windows environment variables
        windows_vars = ['USERPROFILE', 'COMPUTERNAME', 'USERDOMAIN', 'USERNAME']
        for var in windows_vars:
            if var in os.environ:
                logger.info(f"{var}: {os.environ[var]}")
    else:
        logger.info(f"Running on {platform.system()} (non-Windows environment)")
